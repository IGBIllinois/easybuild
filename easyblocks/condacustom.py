...
class CondaCustom(Binary):
    ...
    def extra_options(extra_vars=None):
	    ...
	    'custom_file': [None, "Custom Conda Version File to pass to 'conda env create'", CUSTOM],
        })
        return extra_vars
    ...
    def extract_step(self):
        ...
        if self.src:
            super(CondaCustom, self).extract_step()
	...
	elif self.cfg['custom_file']:
                #self.set_conda_env()
		cmd = 'conda create --force -y -p %s --file %s' % (self.installdir,self.cfg['custom_file'])
                run_cmd(cmd, log_all=True, simple=True)
    ...
    def make_module_extra(self):
        ...
        txt = super(CondaCustom, self).make_module_extra()
