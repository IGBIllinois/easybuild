...
class RPackageCustom(ExtensionEasyBlock):
    """
    ...
    def extra_options(extra_vars=None):
        ...
        extra_vars.update({
            'install_args': [{}, "R Package install arguments", CUSTOM],
        })
	...
	extra_params = self.__class__.extra_options()	
        super(RPackageCustom, self).__init__(*args, **kwargs)

        self.configurevars = []
        self.configureargs = self.cfg.get('install_args')
        self.ext_src = None
    ...
    def run(self):
        ...
        if self.patches:
            super(RPackageCustom, self).run(unpack_src=True)
        else:
            super(RPackageCustom, self).run()
    ...
    def sanity_check_step(self, *args, **kwargs):
        ...
        """
        return super(RPackageCustom, self).sanity_check_step(EXTS_FILTER_R_PACKAGES, *args, **kwargs)

    def make_module_extra(self):
        """Add install path to R_LIBS"""
        extra = self.module_generator.prepend_paths("R_LIBS", [''])  # prepend R_LIBS with install path
        return super(RPackageCustom, self).make_module_extra(extra)
